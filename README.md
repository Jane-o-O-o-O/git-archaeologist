...
### v1.2.0 新增
- **🕰️ 陈旧分支检测** — `stale-branches` 命令找出长期未更新的分支，支持 `--days` 阈值
- **📊 标签间统计** — `tag-stats` 命令分析相邻标签之间的变更（发布分析），含 commits/增删行/文件数/作者数
- **🔍 Commit 详情** — `inspect` 命令详细分析单个 commit，含文件级 diff、父 commit、完整消息
- **📏 最大文件** — `largest` 命令查找仓库中行数最多的文件，支持 `--top` 限制
